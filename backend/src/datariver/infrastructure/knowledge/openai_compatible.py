            ...
            "messages": [
                {
                    ...
                    "content": (
                        ...
                        "underscores as allowed by the schema contract. Every stable_element_id "
                        "must be globally unique across classes, properties and relations. Every "
                        "canonical_name must be case-insensitively unique within its own kind. "
                        "When the same concept appears more than once, emit only the single most "
                        "strongly grounded element. Properties must reference "
                        ...
                        "When the JSON document encoded in the request string has top-level "
                        "contract_version KNOWLEDGE_STUDIO_CATALOG_GROUNDING_INFERENCE_V1, emit "
                        "exactly one Class "
                        "using asset.expected_class_id and then exactly one Property per fields "
                        "entry in array order using expected_property_id. Every Property parent "
                        "must be asset.expected_class_id; the Class and every Property must copy "
                        "asset.asset_id into metadata_reference_id; emit no Relations and do not "
                        "invent or substitute grounding values. "
