...
def _grant_response(value: ConsumerGrantRecord) -> ConsumerGrantResponse:
    return ConsumerGrantResponse(
        ...
        consumer_issuer=value.consumer_issuer,
...
async def create_consumer_grant(
    ...
    grant = await service.create_grant(
        product=product,
        workspace_id=context.workspace_id,
        consumer_subject_id=payload.consumer_subject_id,
...
async def retired_authorize_invocation(
    product_id: UUID,
    payload: ApiInvocationAuthorizeRequest,
    context: ContextDep,
) -> JSONResponse:
    del product_id, payload, context
    return JSONResponse(
        status_code=410,
        media_type="application/problem+json",
        headers={"Cache-Control": "no-store"},
        content={
            "type": "https://datariver.invalid/problems/invocation-reservation-retired",
            "title": "Invocation reservation is retired",
            "status": 410,
            "detail": (
                "Invoke a typed API-product surface directly. Quota is recorded only with "
                "a completed replayable result."
            ),
        },
    )
...
async def invoke_neighbor_analysis(
    ...
    async def build_result(
        ...
        response = NeighborAnalysisResponse(
    ...
    payload_document["edge_types"] = sorted(payload.edge_types)
    result = await service.execute_invocation(
        product=product,
        workspace_id=context.workspace_id,
        consumer_issuer=context.identity.issuer,
    ...
    return NeighborAnalysisResponse.model_validate(result.result_document)
...
async def invoke_snapshot(
    ...
    async def build_result(
        ...
        if stored is None:
            raise NotFoundError("The API product's pinned graph release is unavailable.")
        release, snapshot = stored
        response = KnowledgeSnapshotResponse(
            release=KnowledgeReleaseResponse(
                id=release.release_id,
                graph_id=release.graph_id,
                release_no=release.release_no,
            ...
            filtered=(
                len(snapshot.nodes) != release.node_count
                or len(snapshot.edges) != release.edge_count
            ),
        )
        return response.model_dump(mode="json")
    ...
    result = await service.execute_invocation(
        ...
        operation="SNAPSHOT_V1",
        result_type="SNAPSHOT_V1",
        payload_document=payload.model_dump(mode="json"),
        invocation_key=invocation_key,
        subject=context.subject,
        environment=context.environment,
    ...
    return KnowledgeSnapshotResponse.model_validate(result.result_document)
...
async def invoke_chat(
    ...
    async def build_result(
        ...
        response = ApiProductChatInvokeResponse(
    ...
    result = await service.execute_invocation(
        ...
        operation="CHAT_LOCAL_V1",
        result_type="CHAT_LOCAL_V1",
        payload_document=payload.model_dump(mode="json"),
        invocation_key=invocation_key,
        subject=context.subject,
        environment=context.environment,
    ...
    return ApiProductChatInvokeResponse.model_validate(result.result_document)
