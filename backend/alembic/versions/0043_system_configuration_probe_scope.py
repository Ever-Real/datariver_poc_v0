"""Align authenticated connector probe evidence vocabulary.

Revision ID: 0043
Revises: 0042
Create Date: 2026-07-23
"""

from collections.abc import Sequence

import sqlalchemy as sa
from alembic import op

revision: str = "0043"
down_revision: str | Sequence[str] | None = "0042"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None

_CONSTRAINT = "ck_external_service_profile_versions_test_scope_vocabulary"
_LEGACY_SCOPES = (
    "HTTP_HEALTH",
    "MODEL_DISCOVERY",
    "MODEL_INFERENCE",
    "EMBEDDING_INFERENCE",
    "AUTHENTICATED_QUERY",
    "REDIS_PING",
)
_CURRENT_SCOPES = (*_LEGACY_SCOPES, "REDIS_POLICY", "S3_HEAD_BUCKET")


def _scope_sql(scopes: tuple[str, ...]) -> str:
    return (
        "test_scope IS NULL OR test_scope IN (" + ", ".join(f"'{scope}'" for scope in scopes) + ")"
    )


def _scope_definition(scopes: tuple[str, ...]) -> str:
    values = ", ".join(f"'{scope}'::character varying" for scope in scopes)
    return f"CHECK (test_scope IS NULL OR (test_scope::text = ANY (ARRAY[{values}]::text[])))"


_LEGACY_SCOPE_SQL = _scope_sql(_LEGACY_SCOPES)
_CURRENT_SCOPE_SQL = _scope_sql(_CURRENT_SCOPES)
_LEGACY_SCOPE_DEFINITION = _scope_definition(_LEGACY_SCOPES)
_CURRENT_SCOPE_DEFINITION = _scope_definition(_CURRENT_SCOPES)


def _constraint_definition() -> str | None:
    definition = op.get_bind().scalar(
        sa.text(
            """
            SELECT pg_get_constraintdef(constraint_state.oid, true)
            FROM pg_constraint AS constraint_state
            WHERE constraint_state.conrelid =
                    'platform.external_service_profile_versions'::regclass
              AND constraint_state.conname = :constraint_name
            """
        ),
        {"constraint_name": _CONSTRAINT},
    )
    return definition if isinstance(definition, str) else None


def _replace_scope_constraint(
    *,
    expected_definition: str,
    next_sql: str,
    next_definition: str,
) -> None:
    definition = _constraint_definition()
    if definition != expected_definition:
        raise RuntimeError("The connector probe scope constraint is missing or malformed.")
    op.drop_constraint(
        op.f(_CONSTRAINT),
        "external_service_profile_versions",
        schema="platform",
        type_="check",
    )
    op.create_check_constraint(
        op.f(_CONSTRAINT),
        "external_service_profile_versions",
        next_sql,
        schema="platform",
    )
    if _constraint_definition() != next_definition:
        raise RuntimeError("The connector probe scope constraint replacement was not canonical.")


def upgrade() -> None:
    definition = _constraint_definition()
    if definition == _CURRENT_SCOPE_DEFINITION:
        return
    if definition != _LEGACY_SCOPE_DEFINITION:
        raise RuntimeError("The connector probe scope constraint is missing or malformed.")
    _replace_scope_constraint(
        expected_definition=_LEGACY_SCOPE_DEFINITION,
        next_sql=_CURRENT_SCOPE_SQL,
        next_definition=_CURRENT_SCOPE_DEFINITION,
    )


def downgrade() -> None:
    current_evidence = op.get_bind().scalar(
        sa.text(
            "SELECT count(*) FROM platform.external_service_profile_versions "
            "WHERE test_scope IN ('REDIS_POLICY', 'S3_HEAD_BUCKET')"
        )
    )
    if int(current_evidence or 0) != 0:
        raise RuntimeError(
            "Downgrade would falsify connector probe evidence; archive or explicitly "
            "invalidate the affected revisions before retrying."
        )
    _replace_scope_constraint(
        expected_definition=_CURRENT_SCOPE_DEFINITION,
        next_sql=_LEGACY_SCOPE_SQL,
        next_definition=_LEGACY_SCOPE_DEFINITION,
    )
