...
def _effective(bundle: object, project: str) -> object:
...
def _marker(path: Path, release: object) -> None:
...
def test_unified_state_machine_and_non_destructive_failed_install_recovery(
    ...
    try:
        ...
        assert deploy.prove_failed_install_recoverable(
            runner,
            release,
            inspection,
            residual,
        ...
        with deploy.private_effective_environment(recovered.effective) as env_file:
            ...
            runner.run([
                *deploy._postgres_local_command(
                ...
                "--command",
                "INSERT INTO poc_state(scope, value) VALUES ('integration-durable', '{}'::jsonb);",
            ])
        ...
        assert deploy.classify_target_state(deploy.inspect_target_inventory(
            runner,
