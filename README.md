...
태그 이후의 배포 도구에서는 기존 `39080`·`39083`의 상태 비교와 결과 항목을 제외했습니다. 이 변경은 고정 태그의 배포 성공 기록과 구분하며, 실제 기능의 Smoke 6단계와 배포 대상의 검증 범위는 유지합니다.
...
빌드·배포 과정에서 기존 `39080`·`39083` 컨테이너의 상태를 수집·비교하지 않으며, 최종 결과와 `acceptance.json`에도 두 포트의 `UNTOUCHED` 판정을 기록하지 않습니다. 선택한 배포 대상의 포트 소유권, Compose 구성, 이미지·소스 일치, 기존 볼륨·환경 보존, 인증·provider·기능 검증은 유지합니다. 기본 대상은 `datariver-dev:39091`이며 `--port 39083`은 기존 PREP를 직접 재배포하는 명시적 선택이므로 독립 설치에서는 사용하지 않습니다.
