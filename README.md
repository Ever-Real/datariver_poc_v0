...
거부한다. Linux x64에서는 lockfile에 고정된
`@rolldown/binding-linux-x64-gnu` native optional package와 실제 `.node` 로딩도 검사한다.
누락 시 platform package를 `package.json`에 직접 추가하지 말고 동일 Node 환경에서
`npm --prefix frontend ci --include=optional --no-audit --no-fund`로 전체 frozen install을
다시 수행한다.
...
Open `http://localhost:8080`, sign in as `datariver-admin`, and read the generated temporary password from `secrets/keycloak_demo_password`. The first sign-in requires a new password but does not request a mobile OTP. Intranet development defaults `OIDC_HARDWARE_WEBAUTHN_ENABLED=false`, so device enrollment and step-up controls are hidden and high-risk operations remain fail-closed unless an accepted development-only password composition applies. Governance Document publish/archive can use the scoped ADR-0087 path when `ADMIN_PASSWORD_FALLBACK_ENABLED=true` and `DEVELOPMENT_ADMIN_PASSWORD_BYPASS_ENABLED=true`; unrelated high-risk Actions remain denied. To exercise the hardware-specific path, an operator must explicitly set WebAuthn to `true`, disable the password exception, reapply the runtime, enroll the local realm's user-verifying authenticator and complete LoA 2 so the token satisfies the configured ACR, AMR and `auth_time` contract. Bootstrap assigns this active default Workspace, so a verified user does not need to type it after login:
