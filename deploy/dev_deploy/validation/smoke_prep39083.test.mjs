...
const script = resolve('scripts/smoke_prep39083.mjs')
...
async function fixture(k9Mode, {
  chatStatus = 200,
  chatAnswer = "Synthetic representative general explanation.",
    ...
    if (request.url === '/healthz') {
      ...
      if (request.headers.origin !== canonicalOrigin) {
        ...
        json(chatStatus, chatStatus === 200
          ? { route: { selected_mode: 'GENERAL' }, evidence: [], answer: chatAnswer }
          : {
...
test('PREP smoke rejects an empty successful GENERAL provider response', async () => {
  const result = await fixture('deferred', { chatAnswer: '' })
  assert.equal(result.completed.code, 2)
  assert.equal(result.failure.classification, 'PREP_SMOKE_GENERAL_ROUTE_FAILED')
})
