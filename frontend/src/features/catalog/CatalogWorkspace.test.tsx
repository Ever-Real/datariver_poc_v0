...
function defaultRequest(path: string, options?: RequestOptions): Promise<unknown> {
  ...
  if (path === `/catalog/assets/${asset.id}`) return Promise.resolve({ ...asset, ownership: [], glossary_terms: [], tags: ['trusted'], schema_fields: [{ fieldPath: 'wafer_id', type: 'STRING', description: 'identifier' }], quality: {}, source_version: 'source-v7' })
