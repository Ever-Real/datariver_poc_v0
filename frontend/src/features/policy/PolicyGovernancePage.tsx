...
function ClassificationDocument({ policy }: { policy: ClassificationAccessPolicy | null }) {
  ...
  return <section className="policy-body"><p>분류별 검색과 Chat의 허용 범위는 이 활성 정책과 서버 ABAC 결정에 따라 제한됩니다.</p><dl className="policy-fact-grid"><div><dt>정책 번호</dt><dd>{policy.policy_number}</dd></div><div><dt>상태</dt><dd>{policy.state}</dd></div><div><dt>관할</dt><dd>{policy.required_jurisdiction}</dd></div><div><dt>제한 검색 최대 기간</dt><dd>{policy.restricted_search_grant_maximum_days}일</dd></div></dl><h2>분류별 제어</h2><div className="policy-rule-table"><div className="policy-rule-head"><span>Classification</span><span>Search</span><span>Chat</span><span>Provider profile</span></div>{policy.rules.map((rule) => <div key={rule.classification}><strong>{rule.classification}</strong><span>{rule.search_mode}</span><span>{rule.chat_mode}</span><code>{rule.provider_profile_version_id ?? '—'}</code></div>)}</div></section>
}
