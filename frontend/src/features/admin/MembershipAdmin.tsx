...
export function MembershipAccessAdmin({
  ...
  const provisionUser = async () => {
    if (
      !canProvision
      || createBusy
      || (!pocOpenAccess && newUser.temporaryPassword !== newUser.passwordConfirmation)
    ) return
    const payload = {
      ...
      role_id: null,
      temporary_password: pocOpenAccess ? '' : newUser.temporaryPassword,
    }
    setCreateBusy(true)
    try {
  ...
  return <>
    <section className="panel">
      <div className="section-heading"><div><h3>User 관리</h3><p className="muted">{pocOpenAccess ? 'POC 사용자 프로필, 업무 역할과 CR 활동을 관리합니다.' : '사용자를 선택해 프로필, 데이터·화면 접근 Role, CR 활동과 인증 복구를 관리합니다.'}</p></div><div className="action-row"><button type="button" className="button button-secondary" onClick={() => void loadMembers()}>{messages.refresh}</button>{onOpenRenewals && <button type="button" className="button button-secondary" onClick={onOpenRenewals}>계정 갱신</button>}{onOpenRoleManagement && <button type="button" className="button button-secondary" onClick={onOpenRoleManagement}>Role 관리</button>}<button type="button" className="button" disabled={!canProvision} title={canProvision ? (pocOpenAccess ? '인증 계정 없이 POC 사용자 프로필을 생성합니다.' : '인증 계정과 Workspace 멤버십을 함께 생성합니다.') : '서버가 현재 세션에 사용자 등록 권한을 허용하지 않았습니다.'} onClick={openCreate}>사용자 등록</button></div></div>
    ...
    />
    <Dialog open={createOpen} title="사용자 등록" description={pocOpenAccess ? '인증 계정 없이 현재 POC에 사용할 로컬 사용자 프로필을 생성합니다.' : '인증 계정과 현재 Workspace 멤버십을 하나의 통제된 작업으로 생성합니다.'} onRequestClose={closeCreate} footer={<><button type="button" className="button button-secondary" disabled={createBusy} onClick={closeCreate}>취소</button><button type="button" className="button" disabled={createBusy || !newUser.username.trim() || !newUser.email.trim() || !newUser.firstName.trim() || !newUser.lastName.trim() || (!pocOpenAccess && (newUser.temporaryPassword.length < 12 || newUser.temporaryPassword !== newUser.passwordConfirmation))} onClick={() => void provisionUser()}>{createBusy ? '등록 중…' : '사용자 등록'}</button></>}>
      <div className="grid gap-3 md:grid-cols-2"><p className="callout m-0 md:col-span-2"><strong>POC 접근 정책</strong> · Role과 System scope는 서버 access authority에서 판정하며, 화면 표시와 무관하게 backend가 최종 권한을 강제합니다.{pocOpenAccess ? ' 이 화면의 프로필 생성은 interactive login credential을 생성하지 않습니다.' : ''}</p><label className="grid gap-1 text-xs font-bold">사용자명<input required minLength={3} maxLength={64} autoComplete="off" value={newUser.username} onChange={(event) => setNewUser({ ...newUser, username: event.target.value })} placeholder="예: hong.gildong" /></label><label className="grid gap-1 text-xs font-bold">Email<input required type="email" maxLength={320} autoComplete="off" value={newUser.email} onChange={(event) => setNewUser({ ...newUser, email: event.target.value })} /></label><label className="grid gap-1 text-xs font-bold">이름<input required maxLength={100} value={newUser.firstName} onChange={(event) => setNewUser({ ...newUser, firstName: event.target.value })} /></label><label className="grid gap-1 text-xs font-bold">성<input required maxLength={100} value={newUser.lastName} onChange={(event) => setNewUser({ ...newUser, lastName: event.target.value })} /></label><label className="grid gap-1 text-xs font-bold">부서 UUID (선택)<input value={newUser.departmentId} onChange={(event) => setNewUser({ ...newUser, departmentId: event.target.value })} /></label><label className="grid gap-1 text-xs font-bold">업무 역할<select value={newUser.jobFunction} onChange={(event) => setNewUser({ ...newUser, jobFunction: event.target.value })}><option value="">선택</option><option value="developer">developer</option><option value="data_steward">data_steward</option><option value="viewer">viewer</option><option value="admin">admin</option></select></label>{!pocOpenAccess && <><label className="grid gap-1 text-xs font-bold">임시 비밀번호<input required type="password" minLength={12} maxLength={128} autoComplete="new-password" value={newUser.temporaryPassword} onChange={(event) => setNewUser({ ...newUser, temporaryPassword: event.target.value })} /></label><label className="grid gap-1 text-xs font-bold">임시 비밀번호 확인<input required type="password" minLength={12} maxLength={128} autoComplete="new-password" value={newUser.passwordConfirmation} onChange={(event) => setNewUser({ ...newUser, passwordConfirmation: event.target.value })} /></label></>}</div>
    </Dialog>
  </>
}
