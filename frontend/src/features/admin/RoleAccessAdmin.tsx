...
export function RoleManagementDialog({
  ...
  return <Dialog open={open} size="large" title="Role 관리" description="Role 정의와 데이터 접근 수준은 기존의 서버 관리 RBAC API에 저장됩니다." onRequestClose={close} footer={<><button type="button" className="button button-secondary" onClick={close}>닫기</button><button type="button" className="button" disabled={!editingId || !canUpdate || !draft.role_key || !draft.name.trim() || !dataRulesValid} onClick={save}>{editingRole ? 'Role 저장' : 'Role 생성'}</button></>}>
