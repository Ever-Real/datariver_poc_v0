...
describe('RetentionGovernanceAdmin', () => {
  it('explains the provider-neutral lifecycle and switches the three distinct workflows', async () => {
    ...
    const policyTab = screen.getByRole('tab', { name: '보존정책' })
    const holdTab = screen.getByRole('tab', { name: 'Legal Hold' })
    expect(policyTab).toHaveAttribute('tabindex', '0')
    expect(holdTab).toHaveAttribute('tabindex', '-1')
    fireEvent.keyDown(policyTab, { key: 'ArrowRight' })
    expect(holdTab).toHaveFocus()
    expect(holdTab).toHaveAttribute('aria-selected', 'true')
