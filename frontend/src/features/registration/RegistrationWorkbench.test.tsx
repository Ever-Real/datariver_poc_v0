...
describe('Registration workbench', () => {
  ...
  it('uses the v0.3 property table and column grid with an independent manual submission', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '테이블 Tags 추가' }))
    const vocabularyMenu = document.querySelector('.controlled-vocabulary-menu')
    expect(vocabularyMenu).toHaveClass('controlled-vocabulary-menu')
    expect(vocabularyMenu).not.toHaveAttribute('style')
    expect(document.querySelector('.controlled-vocabulary-input-open')).not.toBeNull()
