...
describe('ChatPage', () => {
  ...
  it('keeps bounded discovery results separate from citations without inventing a total', async () => {
    ...
    expect(performance).not.toHaveTextContent('프롬프트 조립')
    const detailSummary = screen.getByText('상세 처리 시간')
    const performanceDetails = detailSummary.closest('details')
    if (!performanceDetails) throw new Error('Expected detailed performance disclosure')
    const detailedPerformance = within(performanceDetails).getByLabelText('상세 처리 시간 항목')
    expect(performanceDetails).not.toHaveAttribute('open')
    expect(detailedPerformance).not.toBeVisible()
