...
| AUTH-1D | `PARTIAL` | Product `2f247107d28716aeba3cfe3fa201fb040ac437e3`: the bounded Table-enforcement slice hydrates current grants and fixed policy once/request and applies exact grant + grade + feature policy before covered Catalog/Search/count/vector/Chat results. Responsible System has no general-read authority; PostgreSQL/memory vector prefilter before ranking; unauthorized Detail is 404, mutation 403; non-Admin Neo4j fails closed. Source/full regressions, representative DEV runtime and a fresh independent Node POC Validator passed. External LLM/embedding/reranker were `ECONNREFUSED`, all current Tables were `normal`, and graph/provider/deleted-grade/unbound Knowledge/Governance/Quality surfaces remain open, so overall 1D is not complete. Evidence: `.orchestration/evidence/DEV-PHASE1D-TABLE-ENFORCEMENT-SLICE-RUNTIME.md`. |
...
| PHASE 1D | `PARTIAL` | Product `2f247107d28716aeba3cfe3fa201fb040ac437e3`: bounded request-time Table enforcement is source-verified and representative DEV-runtime verified across covered Catalog/Search/count/vector/Chat/lineage/mutation paths. External semantic providers, higher-grade canonical negatives, Neo4j provenance, provider-wide traversal/totals, deleted-grade, unbound Knowledge/Governance and Quality/GX remain open; do not promote overall 1D to complete. |
