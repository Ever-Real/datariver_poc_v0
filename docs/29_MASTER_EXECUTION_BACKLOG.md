...
Current Product boundary `2f247107d28716aeba3cfe3fa201fb040ac437e3` is validated through
`.orchestration/evidence/DEV-PHASE1D-TABLE-ENFORCEMENT-SLICE-RUNTIME.md` and the preceding phase
evidence.
...
| AUTH-1B-R | `PARTIAL` | The current Node POC keeps 15 capabilities and request-time access authority while separating explicit Table grants/security grades from Responsible System. The bounded AUTH-1D slice now prefilters covered Catalog/Chat/vector paths; Neo4j provenance, provider-wide traversal/totals and coarse unbound documents remain open under AUTH-1D. |
...
| AUTH-1D | `PARTIAL` | Product `2f247107d28716aeba3cfe3fa201fb040ac437e3`: the bounded Table-enforcement slice hydrates current grants and fixed policy once/request and applies exact grant + grade + feature policy before covered Catalog/Search/count/vector/Chat results. Responsible System has no general-read authority; local result/count and PG/memory vector prefilter before matching/ranking; unauthorized Detail is 404, mutation 403; non-Admin Neo4j fails closed. Gap reduction proved no-N+1 at 10/100/1,002 Tables and diagnosed the former provider refusal as DEV container-loopback configuration drift; existing Chat/embedding/reranker direct contracts now return 200. Actual DataHub-bound semantic E2E remains `IMPLEMENTED_NOT_VERIFIED` because the Web DataHub binding is absent; every current canonical Table is still `normal`, Neo4j has no canonical Table URN provenance, provider traversal/totals, deleted grade, unbound Knowledge/Governance and Quality/GX remain open. Evidence: `.orchestration/evidence/DEV-PHASE1D-TABLE-ENFORCEMENT-SLICE-RUNTIME.md`, `.orchestration/evidence/DEV-PHASE1D-CLOSEOUT-GAP-REDUCTION.md`. |
...
| PHASE 1D | `PARTIAL` | Product `2f247107d28716aeba3cfe3fa201fb040ac437e3`: local Catalog/Search/Tree/Detail/autocomplete/facet/count/dashboard/Monitoring and memory-vector enforcement are `COMPLETE_RUNTIME_VERIFIED`; request hydration is constant-count rather than per-Table. Existing Chat/embedding/reranker direct contracts are repaired and healthy, but actual DataHub-bound vector/AUTO/citation, a canonical `credential`/`restricted` Table sample, provider-wide traversal/totals, Neo4j URN provenance, deleted-grade history, unbound Knowledge/Governance and Quality/GX remain `IMPLEMENTED_NOT_VERIFIED`/`PARTIAL`/`BLOCKED` as applicable. The smallest next gate is a reviewed DEV DataHub binding and approved disposable higher-grade TABLE lifecycle; do not promote overall 1D or start another Product slice without authorization. |
