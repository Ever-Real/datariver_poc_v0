...
| Current controlled phase | Product `038b7ffa6b06666985664d480340b9010fe1fdd9`: Airflow/MinIO DEV gate plus Registration authorization/preparation and manual-metadata apply are runtime-verified; GX and Registration overall remain partial. |
...
Current Product boundary `038b7ffa6b06666985664d480340b9010fe1fdd9` preserves the PHASE 1D-R
...
| P4 | Registration | Authorization/preparation and manual-metadata apply `COMPLETE_RUNTIME_VERIFIED`; overall `PARTIAL` for remaining bulk candidate-to-CR/apply acceptance |
...
- Registration's existing preparation/candidate flow now enforces role + grant + grade + fixed
  ...
  apply also passed exact Product receipts. Overall Registration stays `PARTIAL` only for the
...
| PHASE 1D | `PARTIAL` overall | Product `91ca4db7ca792566b7765f3366036b1d8bed2869` froze bounded local enforcement, canonical higher-grade Table matrix, PostgreSQL/memory pre-ranking, Product General/Vector/AUTO/context/citation and deterministic provider restart as `COMPLETE_RUNTIME_VERIFIED`; Product `038b7ffa6b06666985664d480340b9010fe1fdd9` preserves that baseline and completes bounded Registration authorization/preparation plus manual-metadata receipt compatibility. Provider-wide traversal/totals, Neo4j URN provenance, deleted-grade history, unbound Knowledge/Governance and Quality/GX retain their named `PARTIAL`/`BLOCKED` states. Account/Auth core is a completed baseline; do not falsely promote those surfaces or keep expanding core auth ahead of their owning product features. |
