...
| `POST /admin/access-roles` | `admin.manage` + recent hardware WebAuthn | create one workspace Role from the server-canonical human Action catalog and bounded System/Domain UUID scopes; the 5 service-principal Actions, credentials and arbitrary policy expressions are not accepted |
