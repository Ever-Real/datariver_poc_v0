...
| `POST /admin/identity-users` | eligible human security administrator + recent hardware WebAuthn + enabled governed Keycloak adapter | idempotently create a disabled marked Keycloak identity, temporary `UPDATE_PASSWORD` credential and canonical six-month Workspace membership, optionally from an active `HUMAN_ROLE`, then enable the identity. Canonical Admin is rejected before provider mutation; the password is excluded from request hash, DB, outbox and response. |
...
There is no Canonical Admin bind, rebind, revoke or self-approval HTTP operation. Binding evidence
is read-only and does not change the existing workflow-specific maker/checker decision rules.
