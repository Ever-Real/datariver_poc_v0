...
| `GET /knowledge/domains?classification=&q=&limit=` | `kg.create` + domain scope | single bounded no-store DOMAIN resource for both picker and management table, including nullable management metadata and referenced Asset count |
...
| `PATCH /knowledge/domains/{domain_id}` | `admin.manage` | idempotent ETag-fenced rename of an active DataRiver-managed DOMAIN |
| `DELETE /knowledge/domains/{domain_id}` | `admin.manage` | idempotent ETag-fenced archive; existing Asset/Draft references remain intact and no row is physically deleted |
