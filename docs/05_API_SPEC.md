...
| `GET /knowledge/domains/manage?limit=` | `admin.manage` | bounded no-store list of active DataRiver-managed DOMAIN rows with creator, referenced Asset count, created time and optimistic version |
| `POST /knowledge/domains/manage` | `admin.manage` | idempotently create one workspace DOMAIN; normalized Unicode name, membership-bound creator and source-version hash are server-authored |
| `PATCH /knowledge/domains/manage/{domain_id}` | `admin.manage` | idempotent ETag-fenced rename of an active DataRiver-managed DOMAIN |
| `DELETE /knowledge/domains/manage/{domain_id}` | `admin.manage` | idempotent ETag-fenced archive; existing Asset/Draft references remain intact and no row is physically deleted |
...
| `POST /knowledge/studio/drafts/{draft_id}/tbox/document-proposals` | author `kg.edit` | multipart upload of one allowlisted, bounded document under the exact Draft ETag; after safe parse and create-only filefolder storage, invokes the governed Schema Assistant and returns a typed Proposal with exact object/hash source reference; it never auto-applies |
...
| `GET /knowledge/studio/drafts/{draft_id}/tbox/catalog-sources` | author `kg.edit` + catalog source policy | search only authorized Dataset/Table/View metadata while the Draft is in TBOX; classification ceiling, provider policy and bounded field projection match A-Box source selection |
