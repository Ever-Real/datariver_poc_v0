...
| `GET /capabilities` | `operations.read` | sanitized capability states, optional server-validated external UI links and the ordered Workspace Monitoring tab document. A persisted administrator-approved credential-free HTTP(S) Dashboard Link receives a sandboxed iframe descriptor plus its external fallback; an unsaved deployment default still uses the exact-origin embed gate. No credential-bearing or active-content URL |
| `GET /operations/dashboard` | `dashboard.read` | ordinary home-dashboard facts only: current CR state counts, non-deleted typed DataHub asset/description coverage, active synchronized Glossary Term count and the bounded 200-branch platform/database/schema aggregate. It excludes jobs, uploads, outbox/dead-letter state, capability probes and audit data |
...
| `PUT /admin/monitoring-configuration` | eligible human administrator with fresh `MONITORING_CONFIGURATION_UPDATE`; `If-Match` required | replace and approve for sandboxed iframe presentation up to eight ordered Dashboard label/link/height descriptors. Links may use any credential-free HTTP(S) origin; the server never fetches them and cannot override a target site's own frame-denial headers |
...
| `GET /knowledge/domains/manage?limit=` | `admin.manage` | complete bounded no-store managed DOMAIN inventory for Information Management, including canonical creator display-name/email labels and referenced Asset count; this administration read is distinct from the author picker and never broadens `kg.create` or Subject domain scope |
...
| `POST /admin/identity-users` | eligible human security administrator + current mutation assurance + enabled governed Keycloak adapter | idempotently create a disabled marked Keycloak identity, temporary `UPDATE_PASSWORD` credential, six-month CONFIDENTIAL Workspace membership and Viewer profile assignment, then enable the identity. The compatibility `role_id` field must be null and is rejected before provider mutation; the password is excluded from request hash, DB, outbox and response. |
| `GET /admin/workspace-memberships/{subject_id}/identity-profile` | eligible human security administrator + enabled governed Keycloak adapter | read one active managed human's typed Keycloak profile and Workspace projection with a membership-version `ETag`; no generic provider document is returned |
| `PUT /admin/workspace-memberships/{subject_id}/identity-profile` | `admin.manage` + current mutation assurance + quoted `If-Match` + `Idempotency-Key` | update bounded Keycloak name/email fields, then reconcile local subject and Workspace department/job-function projections through the fixed governed function |
| `PUT /admin/workspace-memberships/{subject_id}/temporary-password` | `admin.manage` + current mutation assurance + quoted `If-Match` + `Idempotency-Key` | set one temporary Keycloak password and revoke the target's current sessions; the credential and derivatives are excluded from request hash, DB, outbox, log and response |
...
| `GET /admin/access-roles/capabilities` | eligible human security administrator with membership-read capability | private/no-store server-canonical 69-Action catalog grouped by service with labels, actor/assignability, default Admin, assurance, reason, pending self-approval and risk metadata; the 5 service-principal Actions are explanatory and cannot be assigned to a human Role |
| `GET /admin/access-roles?q=&status=&limit=&cursor=` | eligible human security administrator with membership-read capability | bounded workspace/filter-bound Role page with current assignment counts and exact four-class rules; server-owned Canonical Admin definitions are explicitly typed and not editable |
| `POST /admin/access-roles` | `admin.manage` + recent hardware WebAuthn | create one `HUMAN_ROLE` from the server-canonical human Action catalog and bounded System/Domain UUID scopes; Canonical Admin, the 5 service-principal Actions, credentials and arbitrary policy expressions are not accepted |
| `PUT /admin/access-roles/{role_id}` | `admin.manage` + recent hardware WebAuthn + quoted `If-Match` | update display metadata or an unused `HUMAN_ROLE` security definition; the Role key is immutable, Canonical Admin is server-managed and security changes fail while assigned |
| `DELETE /admin/access-roles/{role_id}` | `admin.manage` + recent hardware WebAuthn + quoted `If-Match` | deactivate an unassigned `HUMAN_ROLE`; Canonical Admin cannot be deactivated and no row or audit evidence is deleted |
| `PUT /admin/workspace-memberships/{subject_id}/role` | `admin.manage` + recent hardware WebAuthn | assign one active `HUMAN_ROLE`, or remove it, by materializing the governed membership access document; requires `If-Match` and `Idempotency-Key`, prohibits self-change and structurally rejects Canonical Admin |
| `GET /admin/profile-role-policy` | active Workspace member; Admin page remains server-authorized | private/no-store four-tier policy presentation with the exact server Action bundles, service grouping, lifecycle note and System-assignability flag |
| `PUT /admin/workspace-memberships/{subject_id}/profile-role` | `admin.manage` + current governed mutation assurance; Admin transition requires fresh hardware WebAuthn and a different VERIFIED Canonical Admin | replace profile authority with one tier using quoted membership `If-Match`, binding version, reason and `Idempotency-Key`; non-Admin tiers atomically remove custom-Role evidence, Admin uses only the dedicated Canonical binding path, and the last Admin cannot be demoted |
...
| `GET /admin/systems/assignee-candidates?q=&limit=&cursor=` | eligible human security administrator with a valid current OIDC identity | bounded name/email search of current Engineer/Steward, Manager or VERIFIED Admin profiles; Viewer, UNASSIGNED, stale/revoked and service identities are excluded server-side |
...
| `GET /admin/systems/{system_id}/schema-scopes?limit=&cursor=` | eligible human with current VERIFIED Canonical Admin binding | private/no-store, System-version-bound current and inactive schema-wide mappings; provider credentials and asset payloads are never returned |
| `GET /admin/systems/{system_id}/schema-scope-candidates?q=&limit=&cursor=` | eligible human with current VERIFIED Canonical Admin binding | bounded active TABLE/VIEW/DATASET candidates; PUBLIC uses the Workspace boundary, INTERNAL/CONFIDENTIAL require current clearance and Domain scope, and RESTRICTED is excluded; the client selects only `asset_id` |
| `PATCH /admin/systems/{system_id}/schema-scopes` | `admin.manage` + current VERIFIED Canonical Admin + deployment-governed fresh assurance | under a share lock, reapply the same actor visibility predicate and derive schema coordinates from selected asset IDs; atomically upsert/deactivate with quoted System `If-Match`, `Idempotency-Key`, reason, audit/outbox and a new System-version `ETag`; active ownership by another System is `409` |
...
| `GET /admin/workspace-memberships/{subject_id}/access` | eligible human security administrator with a valid current OIDC identity | exact typed full access document plus display metadata, membership version, matching `ETag` and Canonical Admin evidence limited to status, Role/catalog/membership/binding versions and update time; binding UUIDs and hashes are never returned |
...
Canonical Admin bind/rebind/revoke is available only through the protected profile transition: it
requires a different current VERIFIED Admin, fresh hardware WebAuthn, reason/version/idempotency and
the last-Admin guard. Generic Role, identity provisioning and raw membership access routes cannot
create this evidence. No HTTP operation enables workflow self-approval; existing maker/checker
