...
# Server-side, request-time conversation context. No durable checkpoint is created.
CHAT_CONVERSATION_MEMORY_ENABLED=true
# Compression starts after this many completed user turns; valid range 1..100.
CHAT_CONVERSATION_COMPRESSION_START_AFTER_USER_TURNS=3
# Added-context budget; valid range 128..4096 and at least 1024 tokens below
# every enabled Chat provider's configured context window.
...
- Disabled-by-default governance-document projection plane:
  `GOVERNANCE_DOCUMENT_WORKER_ENABLED`, `GOVERNANCE_DOCUMENT_DATABASE_URL`,
  `GOVERNANCE_DOCUMENT_DATABASE_SECRET_REF`, `GOVERNANCE_DOCUMENT_WORKER_POLL_SECONDS`,
  `GOVERNANCE_DOCUMENT_WORKER_LEASE_SECONDS`,
  `S3_GOVERNANCE_DOCUMENT_ACCESS_KEY_FILE` and
  `S3_GOVERNANCE_DOCUMENT_SECRET_KEY_FILE`. The dedicated database login and object-store
  principal are mandatory when the worker is enabled; they are not interchangeable with API,
  migration, knowledge or generic S3 credentials.
...
GOVERNANCE_DOCUMENT_DATABASE_URL
GOVERNANCE_DOCUMENT_DATABASE_SECRET_REF
...
GOVERNANCE_DOCUMENT_WORKER_POLL_SECONDS
GOVERNANCE_DOCUMENT_WORKER_LEASE_SECONDS
...
S3_GOVERNANCE_DOCUMENT_ACCESS_KEY_FILE
S3_GOVERNANCE_DOCUMENT_SECRET_KEY_FILE
...
CHAT_CONVERSATION_MEMORY_ENABLED
CHAT_CONVERSATION_COMPRESSION_START_AFTER_USER_TURNS
CHAT_CONVERSATION_CONTEXT_MAX_TOKENS
CHAT_COMPOSITION_PROVIDER_PROFILE_VERSION_ID
