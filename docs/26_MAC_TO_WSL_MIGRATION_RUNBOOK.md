...
`--pull never --no-build`. No additional flag is required for the normal pre-state command.
`--reuse-local-images` forces this path when a state file exists; the older `--connected-build`
spelling remains an alias. The path preserves volumes and does not create or claim offline release
evidence. Managed offline acceptance continues to require the recorded manifest, checksums and
image IDs.

OIDC remains the canonical identity boundary. Removing it would replace the issuer/subject,
Workspace membership, ABAC and RLS identity contract rather than merely hide a login screen.
For preparation-PC-only browser development, derive a source environment with
`bootstrap.sh --host-development` without `--intranet-source-host`; local Keycloak then uses
`http://localhost:18081` and no DNS, certificate, Nginx or Windows inbound rule is required.

Source Vite requires the checked-in frontend engine contract, Node.js `>=22.19.0`. A successful
`npm ci` under an older runtime is not startup evidence: Vite 8 contains syntax that old Node
loaders cannot parse. `dev_host.sh start` checks the resolved WSL `node --version` before starting
any process and reports the selected binary. A closed-network host must receive the official Linux
x64 Node runtime through the approved software channel; Mac arm64 Node binaries are not portable
to WSL amd64.

Vite 8 also loads the lock-pinned optional native package
`@rolldown/binding-linux-x64-gnu@1.1.5`. Reusing `node_modules` installed under another platform or
with optional dependencies omitted produces `Cannot find module
'../rolldown-binding.linux-x64-gnu.node'`. On WSL, rerun the complete frozen install under the final
Node runtime with `npm --prefix frontend ci --include=optional --no-audit --no-fund`; do not add a
Linux-only package to cross-platform `package.json`. Offline installation succeeds only when that
exact tarball is present in the transferred npm cache or internal mirror.
...
If durable PDF analysis is selected, require current Alembic head `0063` (the durable job schema was
...
Require `0063 (head)` after migration. Revision `0054` refuses a missing, privileged, BYPASSRLS or
