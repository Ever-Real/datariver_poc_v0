...
writers against the same database/Redis. Preserve PostgreSQL, Keycloak and Redis volumes.
`compose.source-host.yaml` must recreate PostgreSQL/Keycloak with loopback host publications. A
PostgreSQL listing of `5432/tcp` alone does not satisfy source migration; the required observation
is `127.0.0.1:5432->5432/tcp`.
