...
| `knowledge.tbox_proposal_jobs` | owner/Draft/block/mode, exact base T-Box and requester-authorization hashes, accepted-manifest or bounded Catalog pin, parser/model/request/pin hashes, eight-state/seven-stage lifecycle, optimistic version, lease fence, terminal Proposal result and optional predecessor | PostgreSQL-canonical durable document/Catalog schema inference request; API can enqueue/read/cancel/retry but cannot claim or succeed it |
