...
| Backend behavior | PASS | 311 pytest tests: prior identity, governance, retention, RLS, search/DataHub, KG, sharing and evidence gates plus permission-prefiltered facets/suggestions, request-snapshot-bound search cursors, governed classification access/provider/grant administration, scoped governance UI capabilities, sanitized external UI links, disabled-first inference contracts, bounded DB-pool metrics and the PgBouncer probe source contract |
| Frontend | PASS | TypeScript build mode, ESLint zero warnings, 12 test files/47 tests including the dense TanStack table, accessible Dialog/Accordion/pagination primitives, governed legacy-aligned top navigation, server-derived external links, workspace-state isolation, search floor/URL synchronization, scoped Admin visibility, classification-policy/provider/grant states and mutation contracts |
| Frontend artifact | PASS | current source build: JS 391.37 kB / gzip 112.42 kB; CSS 22.01 kB / gzip 5.22 kB |
...
| Migration | PASS | current generated `0001` SHA-256 `3213190570a603956582009bd9470399f724b752169f0e29eb38c2801fd0154d`; Alembic sole head and packaged/runtime readiness revision are `0012`. The populated local database upgraded `0011 -> 0012`, reported `0012 (head)`, and host API/APISIX readiness returned 200 after the source processes restarted |
...
| Governed catalog discovery `0012` | PASS (backend/local DB) | search metadata, permission-prefiltered asset type/platform/classification facets, two-character prefix suggestions, policy/projection-aware short-TTL caches and request-snapshot-bound cursors pass strict type/lint/unit/static checks. The local migration, index installation path, OpenAPI exposure and API/APISIX readiness passed; target-distribution EXPLAIN/BUFFERS and the 60-minute soak are not yet evidence |
