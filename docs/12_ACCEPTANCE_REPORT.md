...
| Migration | PASS (source graph) | current generated `0001` SHA-256 `3213190570a603956582009bd9470399f724b752169f0e29eb38c2801fd0154d`; Alembic sole head and packaged/runtime readiness revision are `0012`. The last populated local runtime evidence below is through `0011`; applying and probing `0012` remains a live follow-up |
...
| Governed catalog discovery `0012` | PASS (source/unit only) | search metadata, permission-prefiltered asset type/platform/classification facets, two-character prefix suggestions, policy/projection-aware short-TTL caches and request-snapshot-bound cursors pass strict type/lint/unit/static checks. Target-distribution EXPLAIN/BUFFERS, live migration and 60-minute soak are not yet evidence |
