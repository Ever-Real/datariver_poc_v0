...
| Reranking System TEST | PASS (local/source contract only) | Fixed `POST /v1/rerank` TEST supports the bounded Mac `LOCAL_LLAMA_CPP` route and private `INTRANET_RERANK_V1` route. The former validates finite raw logits; the latter retains mounted-key, TLS/private-host and `[0,1]` score constraints. Both reject invalid ordered score shapes, record `RERANKING_INFERENCE`, and deliberately have no runtime ACTIVATE/consumer. |
