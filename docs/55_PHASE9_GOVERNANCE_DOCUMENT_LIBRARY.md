...
- Decision: [ADR-0080](adr/0080-governance-document-library-and-knowledge-projection.md),
  refined by
  [ADR-0082](adr/0082-governance-document-pgvector-download-and-chat-grounding.md) and
...
Revision `0077` performs no content rewrite. It renames the additive `0075` dimension check to the
same physical identifier generated by SQLAlchemy metadata and canonical `0001`, and refuses to
continue when neither known constraint is present.

Revision `0079` replaces the legacy status/demo surface with managed-document semantics. Governance
opens on **문서 조회**, while **문서 관리** is capability-gated. The three named starter policies
are controlled `STARTER_DOCUMENT` blueprints, not fabricated active rows: each becomes visible only
after ordinary create, attachment, submit and independent approval. Immutable versions own their
parent link, and permission-pruned detail/export responses expose the parent and authorized
children. MinIO continues to use private UUID directories and exact VersionId receipts while
adding distinct readable document/reference basenames. The authorized JSON export deliberately
omits every storage locator and signing detail.
