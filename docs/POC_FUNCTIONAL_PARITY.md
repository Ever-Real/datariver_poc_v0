...
| Change management | Registration with request attachments, review, recoverable changes requested, immutable revision/resubmission, test attachment/result, test approval, final approval, cancellation, and completion. Review approval records its evidence and advances to TESTING as one explicit UI command sequence. TESTING presents only complement and approval request actions; approval remains disabled until the current round has a PASSED typed result | POC PostgreSQL/process memory; object bytes optionally MinIO |
| Quality management | The permission-scoped quality dashboard is the default tab. The asset history tab keeps the shared searchable Resource Tree and live DataHub columns; no fabricated quality scores, rules, runs, or trends | DataHub read |
| Chat | The original dense three-pane conversation/history/evidence composition is retained while true same-origin SSE exposes each real workflow stage immediately. AUTO applies bounded fast rules then a compact typed LLM decision; explicit GENERAL/VECTOR/GRAPH bypass it. Exact table questions load live DataHub detail/columns. Semantic VECTOR searches persisted pgvector immediately, primes a bounded live candidate set when cold and reconciles the complete inventory in the background before optional reranking. GRAPH resolves entities before direction-preserving bounded DataHub lineage and optional Neo4j knowledge edges | DataHub/POC pgvector/Neo4j + configured LLMs |
| Monitoring | Provider probes plus optional exact-origin Grafana dashboard embed | POC server + Grafana |
| POC USER/Admin | Registration, Knowledge and live Glossary remain separate profile items. Account/access, the OPEN feature-permission inventory, system settings and retention/erasure governance are reached through one `관리자메뉴` item and remain four tabs on that page. The nested security-policy view renders the existing four-class redacted `STATIC_FLOOR` contract; no governed policy/provider/grant record is fabricated. User role is developer/data_steward/viewer/admin; System, assignee and DataHub schema scope records are user-created | POC PostgreSQL/process memory + DataHub |
...
| Governance documents | No fixture documents; user-created immutable versions support submit, approve/publish or reject, Archive, product blueprints, export/evidence and MinIO attachments when configured. HTML/Markdown import is allowlist-sanitized into an editable formatted canvas. HTML file import additionally retains a bounded static CSS subset through a revalidated React presentation token; JavaScript, event handlers, URL CSS and embedded content remain disabled | POC PostgreSQL/process memory + MinIO/Neo4j capability gates |
