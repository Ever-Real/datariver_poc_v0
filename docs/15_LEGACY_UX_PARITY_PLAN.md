...
|---|---|
| DataHub lineage detail | Render the fixed-depth, server-authorized graph in the application Lineage tab. A node `상세` modal frames the actual DataHub Lineage page only through a server-built allowlisted URL; the browser never receives a provider token or arbitrary URL and DataHub retains its own session boundary. |
| Grafana monitoring iframe | Use only a configured, server-approved origin and capability. It remains a direct link until matching exact-origin configuration, explicit enablement and SSO/frame-policy evidence produce an available server descriptor; the resulting frame is sandboxed/no-referrer. Absence renders an honest unavailable state, not a hard-coded localhost fallback. |
...
| UX-PAR-002 | Global search | debounced authorized suggestions with preview, minimum length, multi-keyword handoff | SOURCE COMPLETE / EXTERNAL GATE | suggestion and full search share ALL-term fields and bounded match evidence; Workspace-keyed remount, abort/late-discard, listbox keyboard navigation and policy/projection metadata are source-tested. Authenticated reference-viewport/browser acceptance remains external. |
| UX-PAR-003 | Catalog layout | left Resource Tree, middle dense result table, filter/facet bar, paging and export | SOURCE COMPLETE | typed PostgreSQL/Oracle tree, dense real-projection columns, `ALL` search targets, bounded cursor pagination, responsive popover filters for the authorized facet contract, total count, server-authorized CSV/XLSX export and URL-restored Workspace/page are implemented. Isolated export-worker runtime and authenticated visual acceptance remain deployment gates. |
| UX-PAR-004 | Catalog detail | row accordion with table/column metadata and lineage graph | SOURCE COMPLETE | fixed-contract detail/columns, one-line Type/classification/URN/copy identity, a scroll-independent width handle, `Table Details`/`Lineage` tabs, and an authorization-pruned top-to-bottom React lineage canvas are implemented. The fixed-coordinate graph is excluded from ancestor inline-size calculations and its viewport is clipped to the current detail-pane width, so a Lineage tab switch cannot widen the URN, tab strip, accordion or page or produce an outer horizontal scrollbar. It keeps every stage node while wrapping after three columns, and supports clickable names, fixed stage badges, pan, node drag, grid and zoom; selected-state URL/back-forward and the authenticated visual gate remain. |
| UX-PAR-005 | DataHub lineage | selected lineage node opens DataHub-derived lineage detail | SOURCE COMPLETE | the node action opens the actual DataHub Lineage page in a sandboxed modal frame using only the server-authorized allowlisted URL. DataHub SSO/guest/session and framing policy remain deployment gates; no DataRiver or provider service credential is forwarded. |
| UX-PAR-006 | Manual registration | tree selection, table/column edit, controlled term/tag selection, delete proposal | SOURCE COMPLETE | the v0.3 left tree plus `Table Properties` and `Column Schema Specifications` workbench structure is restored over the typed canonical tree/detail. DataHub `SchemaField.label` is displayed separately as read-only Logical Name while editable Description remains a distinct typed field; absent labels render as absent source data. Existing Tag/Term values stay on one scrollable badge line with compact previous/next controls, a far-right `+` opens the floating comma-aware vocabulary search/new-proposal input directly below that control, and remove is exposed on hover/focus. It retains live, hash-fenced description and controlled-metadata change proposals rather than legacy direct provider writes. |
...
| UX-PAR-009 | Change list/detail | dense CR table, accordion/modal detail, review/re-request/test/completion transitions | PARTIAL | a keyboard-operable dense table, freshly authorized accessible detail, immutable target/approval/transition evidence and version-fenced commands are implemented. ADR-0110 replaces the former same-content re-registration with an actual requester-only `CHANGES_REQUESTED` editor that appends a typed round plus new item set; terminal `REJECTED` stays final. The detail dialog lists authorized private attachments and permits TEST evidence only in `TESTING`; authenticated developer/steward browser journeys remain a deployment gate. |
| UX-PAR-010 | CR creation | legacy field loading, `CR-system-date-random4`, urgency and due-date semantics | PARTIAL | server-issued `CR-{safe-system}-{YYMMDD}-{random4}` number and workspace uniqueness are implemented. `신규 CR 신청` opens one independent v0.3-shaped modal with live multi-table search, current table/column values, manual table/column rows, vocabulary suggestions, due date/priority/urgency/security and private multi-file evidence. Existing targets are server re-read and new tables are typed manual intake evidence; authenticated role-journey/browser acceptance remains open. |
| UX-PAR-011 | CR attachments | multiple request/test attachments, re-edit/re-request, authorized download | PARTIAL | request and TEST manifests persist checksums and server-generated `CR-code-KIND-original-serial` keys in the configured private bucket. The browser uploads only through the authorized API, lists manifest metadata and resolves a short-lived download URL after a fresh CR read. Prior-round attachments remain historical; any optional revised REQUEST upload belongs only to the new current round. Authenticated re-edit evidence remains open. |
...
| UX-PAR-018 | Monitoring | full-height observability panel, refresh, approved Grafana link/frame and honest platform capability state | SOURCE COMPLETE | direct link is default; an iframe is possible only from the disabled-first server descriptor after exact-origin, SSO/frame-policy evidence and CSP deployment configuration. Authenticated target Grafana framing remains an environment gate. |
...
| `/` `DashboardPage` | Date filters, asset/quality cards, expandable platform list, governance shortcut, audit summary | The card/grid/expandable platform layout uses `/operations/dashboard` for current typed DataHub counts, description coverage, active synchronized Glossary Terms and CR state, plus `/quality/dashboard` for server-derived coverage and compact Quality KPIs. The ordinary shortcut order is Catalog Search, CR, Governance, Quality Management and AI Copilot; Knowledge and Registration stay off this ordinary panel. Operator capability/outbox and audit facts are not requested or rendered. Period history remains visibly unavailable until its governed read model exists. | PARTIAL |
| `/search` `SearchPage` | Query/suggestions, advanced filters, tree, dense sortable table, paging, Excel, detail/columns, lineage, URN copy, DataHub iframe | `CatalogPage` binds typed table/view, platform/database/schema, owner/domain/term/tag and source-created metadata from the local DataHub projection, with URN copy feedback and a bounded React lineage graph. Terms and Tags appear beside the asset identity and are never inferred when absent. Each column menu sorts or filters the currently loaded page, and the table exposes an explicit horizontal scroll region. Page sizes are 25/50/100; one page action issues one bounded asset request, while facet requests are independent of cursor navigation. Tree/lineage selection opens authorized detail directly, collapsed tree descendants are evicted and one branch is capped at 200 nodes. Filter scope can be selected from the typed tree; server-authorized CSV/XLSX exports use the same fresh authorization/source snapshot and are not built from visible browser rows. Isolated-worker deployment remains an operator gate. | SOURCE COMPLETE |
...
| `/monitoring` `MonitoringPage` | Refresh and Grafana iframe/failure panel | `MonitoringPage` restores the full-height monitoring panel and refreshes server-owned capability data. It exposes the `/capabilities` allowlisted Grafana direct link by default; only an available server descriptor created from exact-origin deployment configuration and SSO/frame-policy evidence renders the sandboxed no-referrer iframe. No localhost fallback or browser URL input exists. | SOURCE COMPLETE |
| `/governance` `GovernancePage` | Document TOC, text/visual tabs, in-place edit | `PolicyGovernancePage` restores the document TOC and text/workflow views over server policy, retention and Legal Hold read models only when the current administrator context includes the corresponding read operations. Ordinary users do not issue those admin-only requests and see an explicit unavailable state. In-place editing and any non-persistent textarea remain absent; policy mutation stays in the separately authorized administration flows. | PARTIAL |
...
| `/admin/users` `UsersPage`, `UserDetailModal`, `UserFormModal`, `SystemsManagementView` | Search/filter, user create/edit/delete, password field, system assignment | `MembershipAccessAdmin` presents a dense, selectable table of the live OIDC subject and Workspace membership (name, job function, department reference, clearance, active state and version), then exposes membership group/action/scope/clearance changes through server validation, assurance, confirmation and audit. The adjacent System tab reads the canonical System master and lets a recent-hardware-WebAuthn administrator replace Developer/Data Steward priority assignments using only server-returned active members, a confirmation, `If-Match`, idempotency and immutable audit evidence. It enforces at least one member per responsibility and no hard-coded rows. `RoleAccessAdmin` adds a simple RBAC template facade that writes the same ETag-guarded membership Access document and preserves server-side ABAC enforcement; it never sends a browser-owned DataHub policy mutation. Generic identity CRUD/password reset/recent-login telemetry remain IdP or separately authorized audit concerns. | PARTIAL |
...
| `/admin/connections` `admin/SystemSettingsPage` | YAML endpoint/auth-secret editor, ping/save for DataHub/Neo4j/Airflow/Grafana/MinIO/LLM | In development only, the administrator System Settings tab retains the v0 service-selector/YAML interaction over an exact server-owned inventory, optimistic versions and credential-free server templates. It saves addresses, model identifiers and non-sensitive options but rejects new password/token/key values; existing legacy sensitive fields are masked in the editor and omitted from the server-built summary. It has no browser-triggered ping, direct provider mutation or production write path. Production remains deployment/provider-profile managed. | PARTIAL (safe development substitution) |
| `/knowledge` `KnowledgeDashboard` | Registry/Studio/Chat shortcuts | `KnowledgePage` supplies the same compact Registry/Studio/knowledge-chat navigation pattern over v1 contracts: Registry creates/lists canonical graphs with operator-chosen contract-bounded graph type/classification and comma-delimited ontology types (no fixed industry ontology), Studio creates typed changesets and appends provenance-bearing node/edge operations, and the chat shortcut opens the existing authorized citation-based Chat page. The graph explorer is release-pinned and ABAC filtered. | PARTIAL |
...
| `/knowledge/ingest` `IngestionStudio` | DB/file source selector, LLM analysis, visual editor, raw Cypher editor/preview, execute and graph preview | Raw Cypher, filesystem-like source selection and direct Neo4j execution are forbidden. Studio instead accepts a typed Node/Edge UPSERT or DELETE with stable UUID, bounded classification and mandatory provenance; server validation, independent review and immutable release publication remain mandatory. Approved source-snapshot extraction is still a future worker capability. | PARTIAL (safe substitution) |
...
| Hard-coded or hostname-rewritten localhost links and raw external iframe | GNB, `SearchPage.tsx`, `MonitoringPage.tsx`, knowledge diff view | `/capabilities` supplies redacted, allowlisted links. The monitoring page opens a configured Grafana link directly unless the server issues its matching-origin, evidence-gated iframe descriptor; safe DataHub embedding likewise requires a server-issued descriptor. |
...
| 5 | knowledge registry, ontology, graph and evaluation | PARTIAL — TYPED STUDIO | Registry/changeset/release/snapshot contracts and provenance-bearing Node/Edge operation UI are present. Backend domain tests cover release, typed operation, validation, bounded traversal and independent review. Source extraction, evaluation datasets and authenticated browser acceptance remain open. |
