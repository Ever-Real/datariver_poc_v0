...
| 3 | manual and bulk registration | IN PROGRESS — TYPED DESCRIPTION/TARGET BINDING | source/unit and live Vite/API/APISIX contract gates pass. New CRs persist server-only target identity/scope evidence; MANUAL description changes use a live provider preview plus opaque target/source ETag and same-transaction share lock. Raw generic/upload proposal actions are deny-by-default hardware-human operations with no ordinary browser form. Remaining typed table/column/domain/term/tag DTOs, worker apply-time requester/policy reauthorization, target serialization/provider CAS, accepted-content binding/correction, authenticated browser E2E and object-manifest evidence remain |
