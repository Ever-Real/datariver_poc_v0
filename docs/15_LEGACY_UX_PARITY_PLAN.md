...
| UX-PAR-010 | CR creation | legacy field loading, `CR-system-date-random4`, urgency and due-date semantics | PARTIAL | server-issued `CR-{safe-platform}-{YYMMDD}-{random4}` number and workspace uniqueness are implemented. `신규 CR 신청` opens an independent typed modal, searches a live target, validates a DataHub description preview and persists due date, priority and urgency. Full multi-target legacy form parity remains open. |
