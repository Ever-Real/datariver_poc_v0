...
| UX-PAR-006 | Manual registration | tree selection, table/column edit, controlled term/tag selection, delete proposal | PARTIAL | tree/detail plus typed dataset-description live preview/create, empty clear, source drift and server classification are implemented; typed column/domain/term/tag DTOs, logical naming and full delete proposal remain |
| UX-PAR-007 | Bulk registration | CSV/XLS upload, validation summary, correction and proposal creation | PARTIAL | the 300-pixel upload panel, typed-profile selector and dark workflow tracker restore the v0.3 visual structure while multipart quarantine, real validation states and version-fenced full-read-back promotion replace simulated browser execution. Forced-RLS preparation evidence, exact accepted-evidence API, bounded parser contract and an authorization-pruned candidate read API exist disabled-first; the UI creates/reads bodyless server preparation and shows actual progress. Runtime parser/target resolution/staging plus the candidate table, preview and proposal controls remain closed, raw proposal UI is absent and READY is visibly non-executable |
...
| 2 | search, Resource Tree, table result, detail and lineage | IN PROGRESS | source/API/FE unit gates, deterministic lineage graph and disabled-first managed export/safe DataHub embed descriptor are implemented; isolated export-worker runtime approval, DataHub SSO/frame evidence, full URL state and authenticated visual gates remain |
| 3 | manual and bulk registration | IN PROGRESS — TYPED DESCRIPTION/PREPARATION | source/unit and prior live Vite/API/APISIX contract gates pass. New CRs persist server-only target identity/scope evidence; MANUAL description changes use a live provider preview plus opaque target/source ETag and same-transaction share lock. BULK now explicitly selects a bounded dataset-description profile, queues/reads one server-owned preparation from exact accepted-byte evidence and restores the v0.3 dark status tracker with actual server state. Its pure streaming parser and authorization-pruned candidate read plane are implemented and tested, but runtime target resolution, lease staging/finalize, candidate table/preview and proposal actions remain closed. Raw generic/upload proposal actions are deny-by-default hardware-human operations with no ordinary browser form. Remaining worker/candidate/provenance UI, typed table/column/domain/term/tag DTOs, worker apply-time requester/policy reauthorization, target serialization/provider CAS, authenticated browser E2E and target object-version evidence remain |
