...
| UX-PAR-006 | Manual registration | tree selection, table/column edit, controlled term/tag selection, delete proposal | PARTIAL | recognizable tree/detail workbench and permission-bound current projection are implemented read-only; typed table/column/domain/term/tag proposal DTOs, logical naming and delete proposal remain |
| UX-PAR-007 | Bulk registration | CSV/XLS upload, validation summary, correction and proposal creation | PARTIAL | drag/drop selection, multipart quarantine, real validation states/summary and advanced governed proposal are implemented; bounded editable preview, exact accepted-content binding and logical naming remain |
...
| 3 | manual and bulk registration | IN PROGRESS — WORKBENCH/TARGET BINDING | source/unit and live Vite/API/APISIX contract gates pass. New CRs persist server-only target identity/scope evidence; reads and approval/forward transitions reauthorize the current target and quarantine legacy unbound items. Typed ordinary-edit DTOs, worker apply-time requester/policy reauthorization, target serialization/provider CAS, accepted-content binding, preview/correction, authenticated browser E2E and object-manifest evidence remain |
