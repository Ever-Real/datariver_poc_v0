...
| `/admin/connections` `admin/SystemSettingsPage` | YAML endpoint/auth-secret editor, ping/save for DataHub/Neo4j/Airflow/Grafana/MinIO/LLM | The administrator System settings tab now lists the exact server-owned integration inventory and only redacted state/management/secret-presence/embed flags. It deliberately has no YAML editor, URL/secret field, persistence mutation or SSRF-capable ping; those require an operator-secret-manager and audited configuration-command contract. | PARTIAL (safe read substitution) |
