...
| UX-PAR-007 | Bulk registration | CSV/XLS upload, validation summary, correction and proposal creation | PARTIAL | drag/drop selection, multipart quarantine, real validation states/summary and version-fenced full-read-back promotion are implemented. Forced-RLS preparation job/receipt/candidate/binding tables and an exact accepted-evidence preparation create/read API exist disabled-first. The parser/candidate/UI/proposal path remains closed, raw proposal UI is removed and accepted content stays visibly non-executable until bounded preview, exact binding and logical naming exist |
